...
class ViewController: UIViewController {
    ...
    var imgOn: UIImage? //켜진 전구 이미지를 가지고 있는 ULLmage 타입의 변수
    var imgOff: UIImage?    //꺼진 전구 이미지를 가지고 있는 ULLmage 타입의 변수
    @IBOutlet weak var imgView: UIImageView!    //이미지 뷰에 대한 아웃렛 변수
    @IBOutlet weak var btnResize: UIButton!     //버튼에 대한 아웃렛 변수
    ...
    @IBAction func btnResizeImage(_ sender: UIButton) {
        ...
        var newWidth:CGFloat, newHeight:CGFloat //확애할 크기의 계산 값을 보관할 변수
        ...
        else {  //false 현재 푹소된 그림일 경우(즉, 타이틀은 확대)
        ...
        // 이미지 뷰의 프레임 크기를 수정된 너비와 높이로 변경함
        imgView.frame.size = CGSize(width: newWidth, height: newHeight)
        
        isZoom = !isZoom    // isZoom 변슈의 상태를 !연산자를 사용하여 반전시킴
    }

    // ON / OFF 스위치에 대한 액션 함수
    @IBAction func switchImageOnOff(_ sender: UISwitch) {
        if sender.isOn{ // 만약 스위치가 On이면
