...
class ViewController: UIViewController {
    ...
    let interval = 1.0  //ㅇ타이머 간격. 1초
    var count = 0   //타이머가 설정한 간격대로 실행되는지 학인하기 위한 변수
    ...
    @IBAction func changeDatePicker(_ sender: UIDatePicker) {
        ...
        lblPickerTime.text =
        "선택시간: " + formatter.string(from: datePickerView.date)  //데이트 파커에서 선택한 날짜를 formatter의 dateFormat에서 설정한 포맷대로 string메서드를 사용하여 문자열로 변환
    }
    
    //타이머가 구동된 후 정해진 시간이 되었을때 실행할 함수
    @objc func updateTime() {
        //count 값을 문자영로 변환하여 lblCurrentTime.text에 출력
        // lblCurrentTime.text = String(count)
        // count = count + 1    count 값을 1 증가
        
        let date = NSDate() // 현재 시간을 가져옴
        
        //DateFormatter라는 클래스의 함수 formatter를 선언
        let formatter = DateFormatter()
        
        //상수 formatter의 dateFormat 속성을 설정
        formatter.dateFormat = "yyyy-MM-dd HH:mm:ss EEE"
        //현재 날짜(data)를 formatter의 dateFormat에서 설정한 포멧대로
        //string 메서드를 상요하여 문자열로 변환
